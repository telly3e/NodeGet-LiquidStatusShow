...
function parseMeta(raw: Record<string, unknown>): NodeMeta {
    ...
    return {
        ...
        tags: Array.isArray(raw.metadata_tags)
            ? raw.metadata_tags.filter(Boolean)
            : [],
        hidden: Boolean(raw.metadata_hidden),
        virtualization: raw.metadata_virtualization
            ? String(raw.metadata_virtualization)
            : "",
        lat: Number.isFinite(lat) ? lat : null,
        lng: Number.isFinite(lng) ? lng : null,
        order: Number.isFinite(order) ? order : 0,
        price: Number.isFinite(price) ? price : 0,
        priceUnit: raw.metadata_price_unit
            ? String(raw.metadata_price_unit)
            : "$",
        priceCycle: Number.isFinite(cycle) && cycle > 0 ? cycle : 30,
        expireTime: raw.metadata_expire_time
            ? String(raw.metadata_expire_time)
            : "",
    };
}
...
function sampleFrom(row: DynamicSummary): HistorySample {
    ...
    return {
        ...
        mem:
            memTotal && row.used_memory != null
                ? (row.used_memory / memTotal) * 100
                : null,
        disk:
...
export function useNodes(config: SiteConfig | null) {
    ...
    useEffect(() => {
        ...
        const bootstrap = async () => {
            ...
            for (const { source, rows } of agentsRes.ok) {
                ...
                for (const uuid of uuids)
                    seed.set(uuid, blankAgent(uuid, source));
            }
            setAgents(seed);
                    ...
                    setAgents((prev) => {
                        ...
                        if (meta.status === "fulfilled" && meta.value) {
                            ...
                            for (const row of meta.value) {
                                ...
                                if (!bucket)
                                    grouped.set(row.namespace, (bucket = {}));
                                bucket[row.key] = row.value;
                            }
                            for (const uuid of uuids) {
                                const cur =
                                    next.get(uuid) ??
                                    blankAgent(uuid, entry.name);
                                next.set(uuid, {
                                    ...cur,
                                    meta: parseMeta(grouped.get(uuid) ?? {}),
                                });
                            }
                        }
                        ...
                        if (stat.status === "fulfilled" && stat.value) {
                            for (const row of stat.value) {
                                ...
                                const cur =
                                    next.get(row.uuid) ??
                                    blankAgent(row.uuid, entry.name);
        ...
        const tickDynamic = async () => {
            if (document.visibilityState !== "visible" || dynamicInFlight)
                return;
            ...
            try {
                ...
                await Promise.allSettled(
                    pool.entries.map(async (entry) => {
                        ...
                        if (!uuids.length) return;
                        try {
                            const rows = await dynamicSummaryMulti(
                                entry.client,
                                uuids,
                                DYNAMIC_FIELDS,
                            );
                ...
                setHistory((prev) => {
                    ...
                    for (const row of updates) {
                        ...
                        const dedup =
                            arr.length && arr[arr.length - 1].t === sample.t
                                ? arr
                                : arr.concat(sample);
